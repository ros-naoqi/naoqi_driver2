...
MovetoSubscriber::MovetoSubscriber(const std::string& name,
                                   const std::string& topic,
                                   const qi::SessionPtr& session,
                                   const boost::shared_ptr<tf2_ros::Buffer>& tf2_buffer)
    : BaseSubscriber(name, topic, session), p_motion_(session->service("ALMotion").value()),
...
void MovetoSubscriber::reset(rclcpp::Node* node)
{
  sub_moveto_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
      topic_, 10, std::bind(&MovetoSubscriber::callback, this, std::placeholders::_1));
...
void MovetoSubscriber::callback(const geometry_msgs::msg::PoseStamped::SharedPtr pose_msg)
{
  if (pose_msg->header.frame_id == "odom")
  {
    ...
    bool canTransform =
        tf2_buffer_->canTransform("base_footprint", "odom", tf2::get_now(), tf2::Duration(2));
    ...
    try
    {
      tf2_buffer_->transform(*pose_msg, pose_msg_bf, "base_footprint", tf2::get_now(), "odom");

      double yaw = helpers::transform::getYaw(pose_msg_bf.pose);

      std::cout << "odom to move x: " << pose_msg_bf.pose.position.x
                << " y: " << pose_msg_bf.pose.position.y << " yaw: " << yaw << std::endl;
      ...
      p_motion_.async<void>(
          "moveTo", pose_msg_bf.pose.position.x, pose_msg_bf.pose.position.y, yaw);
    }
    catch (const tf2::LookupException& e)
    {
      std::cout << e.what() << std::endl;
      std::cout << "moveto position in frame_id " << pose_msg->header.frame_id
  ...
  else if (pose_msg->header.frame_id == "base_footprint")
  {
    ...
    std::cout << "going to move x: " << pose_msg->pose.position.x
              << " y: " << pose_msg->pose.position.y << " yaw: " << yaw << std::endl;
    ...
    std::cout << "Cannot reach position expressed in the " << pose_msg->header.frame_id
