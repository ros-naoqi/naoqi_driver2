...
static naoqi_bridge_msgs::msg::RobotInfo& getRobotInfoLocal(const qi::SessionPtr& session)
{
  ...
  std::string hardware_version =
      p_memory.call<std::string>("getData", "RobotConfig/Body/BaseVersion");
  ...
  try
  {
    std::vector<std::vector<qi::AnyValue>> config =
        p_motion.call<std::vector<std::vector<qi::AnyValue>>>("getRobotConfig");

    // TODO, fill with the proper string matches from
    // http://doc.aldebaran.com/2-1/naoqi/motion/tools-general-api.html#ALMotionProxy::getRobotConfig
    ...
    for (size_t i = 0; i < config[0].size(); ++i)
    {
      if (config[0][i].as<std::string>() == "Model Type")
      {
        ...
        {
          std::cout << "Error in robot config variable " << (config[0][i]).as<std::string>()
                    << std::endl;
        }
      }
  ...
  {
    // NAOqi 2.9
    std::cout << "ALMotion.getRobotConfig failed (" /*<< e.what()*/
    ...
    {
      std::cout << "Error getting robot type (with ALRobotModel.getRobotType): " << e.what()
                << std::endl;
    }
    ...
    {
      std::cout << "Error getting head version (with ALRobotModel.getConfig): " << e.what()
                << std::endl;
    }
...
const robot::NaoqiVersion& getNaoqiVersion(const qi::SessionPtr& session)
{
  ...
  {
    std::cerr << "Could not retrieve the version of NAOqi: " << e.what() << std::endl;
  ...
  if (version_numbers.size() != 4)
  {
    std::cerr << "Unconsistent version number for NAOqi, should contain 4 "
              << "elements: " << naoqi_version.text << std::endl;
...
bool isDepthStereo(const qi::SessionPtr& session)
{
  ...
  try
  {
    ...
    if (std::find(sensor_names.begin(), sensor_names.end(), "CameraStereo") != sensor_names.end())
    {
      return true;
    }
