...
const sensor_msgs::msg::CameraInfo& getCameraInfo(int camera_source, int resolution)
{
  ...
  else if (camera_source == AL::kDepthCamera)
  {
    if (resolution == AL::kVGA)
    {
      ...
      RCLCPP_WARN(helpers::Node::get_logger(),
  ...
  std::cout << "no camera information found for camera_source " << camera_source
...
CameraConverter::CameraConverter(const std::string& name,
                                 const float& frequency,
                                 const qi::SessionPtr& session,
                                 const int& camera_source,
                                 const int& resolution,
                                 const bool& has_stereo)
    : BaseConverter(name, frequency, session), p_video_(session->service("ALVideoDevice").value()),
      camera_source_(camera_source), resolution_(resolution),
      // change in case of depth camera
      colorspace_((camera_source_ != AL::kDepthCamera) ? AL::kRGBColorSpace
...
void CameraConverter::reset()
{
  ...
  handle_ = p_video_.call<std::string>(
      "subscribeCamera", name_, camera_source_, resolution_, colorspace_, frequency_);
}
...
void CameraConverter::callAll(const std::vector<message_actions::MessageAction>& actions)
{
  ...
  if (handle_.empty())
  {
    ...
    std::cerr << name_ << "Might be a NAOqi problem. Try to restart the ALVideoDevice."
              << std::endl;
    return;
  }
