...
JointStateConverter::JointStateConverter(const std::string& name,
                                         const float& frequency,
                                         const BufferPtr& tf2_buffer,
                                         const qi::SessionPtr& session)
    : BaseConverter(name, frequency, session), p_motion_(session->service("ALMotion").value()),
      p_memory_(session->service("ALMemory").value()), tf2_buffer_(tf2_buffer)
{
}
...
void JointStateConverter::registerCallback(const message_actions::MessageAction action,
                                           Callback_t cb)
{
...
void JointStateConverter::callAll(const std::vector<message_actions::MessageAction>& actions)
{
  ...
  auto lap = [&step_time](const char* step) {
    ...
    RCLCPP_DEBUG(helpers::Node::get_logger(),
                 "JointState::%s took %ld ns",
                 step,
                 now.nanoseconds() - step_time.nanoseconds());
    step_time = now;
  };
  ...
  // Conversion to double. For some joints like hands, the velocity and torque values do not exist,
  // so a void is returned.
  ...
  std::transform(memory_data.begin(),
                 memory_data.begin() + n_joints,
                 msg_joint_states_.position.begin(),
                 to_double);

  msg_joint_states_.velocity.resize(n_joints);
  std::transform(memory_data.begin() + n_joints,
                 memory_data.begin() + 2 * n_joints,
                 msg_joint_states_.velocity.begin(),
                 to_double);

  msg_joint_states_.effort.resize(n_joints);
  std::transform(memory_data.begin() + 2 * n_joints,
                 memory_data.begin() + 3 * n_joints,
                 msg_joint_states_.effort.begin(),
                 to_double);
  ...
  for (MimicMap::iterator i = mimic_.begin(); i != mimic_.end(); i++)
  {
    ...
    {
      double pos =
          joint_state_map[i->second->joint_name] * i->second->multiplier + i->second->offset;
...
void JointStateConverter::setTransforms(const std::map<std::string, double>& joint_positions,
                                        ...
                                        const std::string& tf_prefix)
{
  ...
  for (std::map<std::string, double>::const_iterator jnt = joint_positions.begin();
       jnt != joint_positions.end();
       jnt++)
  {
    std::map<std::string, robot_state_publisher::SegmentPair>::const_iterator seg =
        segments_.find(jnt->first);
    if (seg != segments_.end())
    {
      seg->second.segment.pose(jnt->second)
          .M.GetQuaternion(tf_transform.transform.rotation.x,
...
void JointStateConverter::setFixedTransforms(const std::string& tf_prefix, const rclcpp::Time& time)
{
  ...
  for (std::map<std::string, robot_state_publisher::SegmentPair>::const_iterator seg =
