 ...
 * thus each possible recorder instance has to implement the virtual functions mentioned in the
 * concept
 */
class Recorder
{
   ...
   */
  template <typename T>
  Recorder(T rec) : recPtr_(boost::make_shared<RecorderModel<T>>(rec))
  {
  }
  ...
  */
  bool isInitialized() const { return recPtr_->isInitialized(); }

  void subscribe(bool state) { recPtr_->subscribe(state); }
   ...
   */
  bool isSubscribed() const { return recPtr_->isSubscribed(); }

  std::string topic() const { return recPtr_->topic(); }
  ...
  void writeDump(const rclcpp::Time& time) { recPtr_->writeDump(time); }

  void setBufferDuration(float duration) { recPtr_->setBufferDuration(duration); }
  ...
  {
    RecorderModel(const T& other) : recorder_(other) {}
    ...
    bool isInitialized() const { return recorder_->isInitialized(); }

    void subscribe(bool state) { recorder_->subscribe(state); }

    bool isSubscribed() const { return recorder_->isSubscribed(); }

    std::string topic() const { return recorder_->topic(); }

    void writeDump(const rclcpp::Time& time) { recorder_->writeDump(time); }

    void setBufferDuration(float duration) { recorder_->setBufferDuration(duration); }
